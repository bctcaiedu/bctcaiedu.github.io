/* ---------------------------------------------------------------
   digital36 · 공통 목차 사이드바 (build.js 가 _src/js/nav.tpl.js 로부터 생성)
   페이지 끝에 <script src="../nav.js"></script> 한 줄만 넣으면 됩니다.
   --------------------------------------------------------------- */
(function () {
  var THIS = document.currentScript;

  /* ====== build.js 가 자동으로 채웁니다 (직접 고치지 마세요) ====== */
  var COURSE = "디지털리터러시";
  var SUB    = "36시간 · 5단원 과정";
  var ORG    = "한국폴리텍대학 분당융합기술교육원";
  var DAYS   = [
    {
      "n": 1,
      "dir": "unit1",
  ...
  function build() {
    ...
    DAYS.forEach(function (d) {
      ...
      html += '<div class="sn-dh"><i>UNIT ' + String(d.n).padStart(2, '0') + '</i><em>' + d.title + '</em></div>';
    ...
    html += '</div><div class="sn-ft">' + ORG + '</div>';
    nav.innerHTML = html;
